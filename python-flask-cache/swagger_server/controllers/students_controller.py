...
def add_student(f, body):  # noqa: E501
    ...
    try:
        item = session.query(Students_instants).filter(
            and_(Students_instants.address == body.address, Students_instants.birthday == body.birthday,
                 Students_instants.email == body.email, Students_instants.facebook == body.facebook,
                 Students_instants.full_name == body.full_name, Students_instants.note == body.note,
                 Students_instants.phone == body.phone)).first()
        if item:
            return jsonify({"message": "the student is exist"}), 400
        new_student = Students_instants(address=body.address, birthday=body.birthday, email=body.email,
                                        facebook=body.facebook, full_name=body.full_name, note=body.note,
                                        phone=body.phone)
...
def del_student_by_id(f, student_id):  # noqa: E501
    ...
    try:
        ...
        current_exam_results = session.query(Exam_results_instants).filter(
            Exam_results_instants.student_id == current_student.id).first()
        current_registration = session.query(Registrations_instants).filter(
            Registrations_instants.student_id == current_student.id).first()
...
def get_all_students(f, key_word=None, page_num=0, records_per_page=20):  # noqa: E501
    ...
    if key_word:
        rows = rows.filter(
            or_(Students_instants.address.like(f'%{key_word}%'), Students_instants.email.like(f'%{key_word}%'),
                Students_instants.full_name.like(f'%{key_word}%'), Students_instants.facebook.like(f'%{key_word}%'),
                Students_instants.phone.like(f'%{key_word}%'), Students_instants.note.like(f'%{key_word}%')))
