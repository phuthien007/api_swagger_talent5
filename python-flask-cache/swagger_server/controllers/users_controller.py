...
def create_user(body):  # noqa: E501
    ...
    try:
        ...
        temp_user = session.query(Users_instants).filter(
            or_(Users_instants.username == username, Users_instants.email == email)).first()
        if temp_user:
            return jsonify({'message': 'User already exists. Please Log in.'}), 202

        user = Users_instants(username=username, email=email, password=generate_password_hash(password),
                              fullname=fullname, role="user")
        session.add(user)
        session.commit()
        temp_user = session.query(Users_instants).filter(
            or_(Users_instants.username == username, Users_instants.email == email)).first()
...
def get_all_users(f, key_word=None, page_num=0, records_per_page=20):  # noqa: E501
    ...
    if key_word:
        rows = rows.filter(
            or_(Users_instants.email.like(f'%{key_word}%'), Users_instants.fullname.like(f'%{key_word}%'),
                Users_instants.username.like(f'%{key_word}%')))
