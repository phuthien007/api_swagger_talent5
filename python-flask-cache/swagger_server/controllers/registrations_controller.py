...
def add_registration(f, body):  # noqa: E501
    ...
    item = session.query(Registrations_instants).filter(
        and_(Registrations_instants.class_id == body._class.id, Registrations_instants.student_id == body.student.id,
             Registrations_instants.status == body.status, Registrations_instants.status == body.status,
             Registrations_instants.create_date == body.create_date)).first()
    if item:
    ...
    try:
        ...
        new_registration = Registrations_instants(class_id=body._class.id, student_id=body.student.id,
                                                  status=body.status, register_day=body.register_day,
                                                  create_date=body.create_date)
        add_data(new_registration)
        item = session.query(Registrations_instants).filter(and_(Registrations_instants.class_id == body._class.id,
                                                                 Registrations_instants.student_id == body.student.id,
                                                                 Registrations_instants.status == body.status,
                                                                 Registrations_instants.status == body.status,
                                                                 Registrations_instants.create_date == body.create_date)).first()
        data = ""
...
def del_registration_by_id(f, class_id, student_id):  # noqa: E501
    ...
    if check_authorization(f, "can_delete_registration_by_id") == False:
        return jsonify({"message": "the user dont has permision to request"}), 400
    item = session.query(Registrations_instants).filter(
        and_(Registrations_instants.class_id == class_id, Registrations_instants.student_id == student_id)).first()
    if not item:
...
def get_all_registrations(f, key_word=None, page_num=0, records_per_page=20):  # noqa: E501
    ...
    if key_word:
        try:
            ...
            rows = rows.filter(
                or_(Registrations_instants.class_id == key_word, Registrations_instants.student_id == key_word))
        except Exception as e:
