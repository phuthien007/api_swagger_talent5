...
def add_class(f, body):  # noqa: E501
    ...
    try:
        item = session.query(Classes_instants).filter(
            and_(Classes_instants.name == body.get('name'), Classes_instants.start_date == body.get('start_date'),
                 Classes_instants.end_date == body.get('end_date'), Classes_instants.status == body.get('status'),
                 Classes_instants.course_id == body.get('course').get('id'),
                 Classes_instants.teacher_id == body.get('teacher').get('id'))).first()
        if item:
            return jsonify({"message": "the class is exist"}), 400
        new_class = Classes_instants(name=body.get('name'), start_date=body.get('start_date'),
                                     end_date=body.get('end_date'), status=body.get('status'),
                                     course_id=body.get('course').get('id'), teacher_id=body.get('teacher').get('id'))
...
def del_class_by_id(f, class_id):  # noqa: E501
    ...
    try:
        ...
        current_exam_result = session.query(Exam_results_instants).filter(
            Exam_results_instants.class_id == class_id).first()
        current_registration = session.query(Registrations_instants).filter(
            Registrations_instants.class_id == class_id).first()
