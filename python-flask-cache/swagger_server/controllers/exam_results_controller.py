...
def add_exam_result(f, body):  # noqa: E501
    ...
    try:
        ...
        item = session.query(Exam_results_instants).filter(
            and_(Exam_results_instants.class_id == body._class.id, Exam_results_instants.student_id == body.student.id,
                 Exam_results_instants.exam_id == body.exam.id, Exam_results_instants.score == score,
                 Exam_results_instants.note == note, Exam_results_instants.result_date == result_date)).first()
        if item:
            return jsonify({"message": "the exam_result is exist"}), 400
        new_item = Exam_results_instants(class_id=body._class.id, student_id=body.student.id, exam_id=body.exam.id,
                                         score=body.score, note=body.note, result_date=body.result_date)
        add_data(new_item)
        item = session.query(Exam_results_instants).filter(
            and_(Exam_results_instants.class_id == body._class.id, Exam_results_instants.student_id == body.student.id,
                 Exam_results_instants.exam_id == body.exam.id, Exam_results_instants.score == score,
