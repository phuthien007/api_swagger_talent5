...
def add_event(f, body):  # noqa: E501
    ...
    try:
        item = session.query(Events_instants).filter(
            and_(Events_instants.name == body.name, Events_instants.create_date == body.create_date,
                 Events_instants.happen_date == body.happen_date, Events_instants.status == body.status,
                 Events_instants.class_id == body._class.id)).first()
        if item:
            return jsonify({"message": "event is exist!!!"}), 400
        new_event = Events_instants(name=body.name, create_date=body.create_date, happen_date=body.happen_date,
                                    status=body.status, class_id=body._class.id)
